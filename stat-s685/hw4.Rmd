...
x.start <- rnorm(20, 1, .1)
# x.start <- rep(1.2, 20)
out <- trust.region(f, g, B, x.start)
summary(out$x)
f(out$x)
sum(g(out$x) ** 2)
...
Since $x$ is bounded in set $\mathcal{B}$ and $\lim \inf ||g_k|| = 0$, there 
exists a subsequence $||g_{k_j}||$ that is monotone decreasing to $0$. 
...
By matching, we can say $u = \frac{B g}{\sqrt{g^\top B g}}$ and 
$v = \frac{B^{-1} g}{\sqrt{g^\top B^{-1} g}}$. Then 
$(u^\top u) (v^\top v) = 
\frac{(g^\top B g) (g^\top B^{-1} g)}
{\sqrt{(g^\top B g) (g^\top B g) (g^\top B^{-1} g) (g^\top B^{-1} g)}}$  
$= \frac{(g^\top B g) (g^\top B^{-1} g)}
