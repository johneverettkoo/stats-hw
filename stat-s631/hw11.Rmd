...
The optimal $\lambda$ is 1.868. However, $\lambda = 2$ is in the 95\% confidence 
interval, and it is the only integer value in the interval. So we will use 
$\lambda = 2$ for this problem.
...
of $\lambda$ fit the data very well.

## Part 3

From part (1), we saw that $\lambda = 2$ is in the 95\% confidence interval 
for maximizing the log-likelihood. We can confirm this visually:

```{r p1_3}
invTranPlot(Distance ~ Speed, data = stopping.df, lambda = 2)
```

The optimal $\lambda$ and $\lambda = 2$ produce very similar results.
